    ...
    ! End of header
    !---------------------------------------------------------------------------------
    SUBROUTINE LELargeBflooded_bin
    USE VARS_MAIN
	USE EnKF_mod
	IMPLICIT NONE
	INTEGER J,K
    ...
    OPEN(LargeBfloodedbin,FILE=INPUT_DIRECTORY // 'LargeBasinFloodedCat.bin',STATUS='old',RECL=NC,FORM='UNFORMATTED',ACCESS='DIRECT') !Input binary file with matrix of correlation among catchments

	DO K=1,NFC
	    READ(LargeBfloodedbin,REC=K)(LargeB_FLOODED(J,K),J=1,NC)
    ENDDO
    
    CLOSE(LargeBfloodedbin)
