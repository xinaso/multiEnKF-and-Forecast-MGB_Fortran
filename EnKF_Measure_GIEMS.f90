 !---------------------------------------------------------------------------------
    ...
    ! Se leerá la matriz Minibacias (columnas) vs tiempo (filas):

    iStat0=FLAGVars(1)*nC*nU+FLAGVars(2)*nC+FLAGVars(3)*nC+FLAGVars(4)*nC+FLAGVars(5)*nC+FLAGVars(6)*nC+FLAGVars(7)*nC*nU+FLAGVars(8)*nC+FLAGVars(9)*nC+FLAGVars(10)*nC+FLAGVars(11)*nC+FLAGVars(12)*nC+FLAGVars(13)*NGC  ! Considerando a variavel Area2
    
    if (iT==1) then
        allocate(HAkf(1,1),Ykf(1),Skf(1,1),Dkf(1,1),Ekf(1,1),Erro_GIEMS_t(1),D2kf(1,1),innov(1),Rkf(1,1),rho(1,1),rhoc(1,1))
    endif
    ...
    if (iT>=iTini_EnKF.and.iT<iTfim_EnKF) then
                ...
                if (allocated(Erro_GIEMS_t))    deallocate(Erro_GIEMS_t)
        ...
        if (nObs_t<2) then
            allocate(HAkf(1,nEns),Ykf(1),Skf(1,nEns),Dkf(1,nEns),Ekf(1,nEns),Erro_GIEMS_t(1),D2kf(1,nEns),innov(1),Rkf(1,1),rho(nStatVar,1),rhoc(1,1))   !Julho
        else    
        
            allocate(Ykf(nObs_t),HAkf(nObs_t,nEns),Skf(nObs_t,nEns),Dkf(nObs_t,nEns),Ekf(nObs_t,nEns),Erro_GIEMS_t(nObs_t),rho(nStatVar,nObs_t),rhoc(nObs_t,nObs_t))
            ...
            do iObs=1,nObs_GIEMS
                if (Obs_GIEMS(iObs)/=-9999.0) then
                    i=i+1
                    Ykf(i)=Obs_GIEMS(iObs)
                    !iC=XminiObs_GIEMS(iObs)
                    iCGIEMS=XsubObs_GIEMS(iObs)
                    ...
                    IF (Local_INDEX==1) THEN
                        ...
                        ! Cuando existe una observacion en la Gran sub cuenca definida para GIEMS
                        do IFC=1,NFC
                            if (IFC==iCGIEMS) THEN
                                rho((m-1)*nC+n+IFC,i)=1
                            else
                                rho((m-1)*nC+n+IFC,i)=0
                            endif
                        enddo
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>            
		    ...
		    do iEns=1,nEns
			    ...
			    do iObs=1,nObs_t
				    ...
				    wran=gasdev(seed)
				    ! Gera vetor de erros
				    Ekf(iObs,iEns)=abs(Ykf(iObs))*wran*Erro_GIEMS_t(iObs)
			    enddo
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            Rkf=0.0
		    do iObs=1,nObs_t
                Rkf(iObs,iObs)=(abs(Ykf(iObs))*Erro_GIEMS_t(iObs))**2
            enddo
    endif
    
	end subroutine EnKF_Measure_GIEMS