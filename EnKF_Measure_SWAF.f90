 !---------------------------------------------------------------------------------
    ...
    if (iT==1) then
        allocate(HAkf(1,1),Ykf(1),Skf(1,1),Dkf(1,1),Ekf(1,1),Erro_SWAF_t(1),D2kf(1,1),innov(1),Rkf(1,1),rho(1,1),rhoc(1,1))
    endif
    ...
    if (iT>=iTini_EnKF.and.iT<iTfim_EnKF) then
                ...
                if (allocated(Erro_SWAF_t))    deallocate(Erro_SWAF_t)
        ...
        if (nObs_t<2) then
            allocate(HAkf(1,nEns),Ykf(1),Skf(1,nEns),Dkf(1,nEns),Ekf(1,nEns),Erro_SWAF_t(1),D2kf(1,nEns),innov(1),Rkf(1,1),rho(nStatVar,1),rhoc(1,1))   !Julho
        else    
        
            allocate(Ykf(nObs_t),HAkf(nObs_t,nEns),Skf(nObs_t,nEns),Dkf(nObs_t,nEns),Ekf(nObs_t,nEns),Erro_SWAF_t(nObs_t),rho(nStatVar,nObs_t),rhoc(nObs_t,nObs_t))
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>            
		    ...
		    do iEns=1,nEns
			    ...
			    do iObs=1,nObs_t
				    ...
				    wran=gasdev(seed)
				    ! Gera vetor de erros
				    Ekf(iObs,iEns)=abs(Ykf(iObs))*wran*Erro_SWAF_t(iObs)
			    enddo
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            Rkf=0.0
		    do iObs=1,nObs_t
                Rkf(iObs,iObs)=(abs(Ykf(iObs))*Erro_SWAF_t(iObs))**2
            enddo

    endif
    
	end subroutine EnKF_Measure_SWAF