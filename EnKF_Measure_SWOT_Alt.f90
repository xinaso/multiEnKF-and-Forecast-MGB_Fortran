 !---------------------------------------------------------------------------------
    ...
    ! End of header
    !---------------------------------------------------------------------------------
    subroutine EnKF_Measure_SWOT_Alt
    use mod_anafunc
	!------------------------------------------------------------------------------
	!-----------------------------------------------------------------------------
    ...
    read(55581,*) i,j,k,(Erro_SWOT_Alt(iObs),iObs=1,nObs_SWOT)
    
    read(5558300,*) i,j,k,(meanERRO_SWOT_Alt(iObs),iObs=1,nObs_SWOT)
    

    iStat0=FLAGVars(1)*nC*nU+FLAGVars(2)*nC+FLAGVars(3)*nC+FLAGVars(4)*nC+FLAGVars(5)*nC+FLAGVars(6)*nC+FLAGVars(7)*nC*nU+FLAGVars(8)*nC  ! Considerando a variavel Hfl
    
    if (iT==1) then
        allocate(HAkf(1,1),Ykf(1),Skf(1,1),Dkf(1,1),Ekf(1,1),Erro_SWOT_Alt_t(1),meanERRO_SWOT_Alt_t(1),D2kf(1,1),innov(1),Rkf(1,1),rho(1,1),rhoc(1,1))       !Julho
    ...
    if (iT>=iTini_EnKF.and.iT<iTfim_EnKF) then   
                ...
                if (allocated(Erro_SWOT_Alt_t))    deallocate(Erro_SWOT_Alt_t)
                if (allocated(meanERRO_SWOT_Alt_t)) deallocate(meanERRO_SWOT_Alt_t)
        ...
        nObs_t=0     
        do iObs=1,nObs_SWOT
            if (Obs_SWOT_Alt(iObs)/=-9999.0) then
        ...
        if (nObs_t<1) then
            allocate(HAkf(1,nEns),Ykf(1),Skf(1,nEns),Dkf(1,nEns),Ekf(1,nEns),Erro_SWOT_Alt_t(1),meanERRO_SWOT_Alt_t(1),D2kf(1,nEns),innov(1),Rkf(1,1),rho(nStatVar,1),rhoc(1,1))   !Julho
                        
        else   
        
            allocate(Ykf(nObs_t),HAkf(nObs_t,nEns),Skf(nObs_t,nEns),Dkf(nObs_t,nEns),Ekf(nObs_t,nEns),Erro_SWOT_Alt_t(nObs_t),meanERRO_SWOT_Alt_t(nObs_t),rho(nStatVar,nObs_t),rhoc(nObs_t,nObs_t))    !Julho
            ...
            do iObs=1,nObs_SWOT
                if (Obs_SWOT_Alt(iObs)/=-9999.0) then
                    i=i+1
                    Ykf(i)=Obs_SWOT_Alt(iObs)
                    iC=XminiObs_SWOT_Alt(iObs)
                    
                    IF (Local_INDEX==1) THEN
                        do m=1,FLAGVars(1)*nU+FLAGVars(2)+FLAGVars(3)+FLAGVars(4)+FLAGVars(5)+FLAGVars(6)+FLAGVars(7)*nU+FLAGVars(8)+FLAGVars(9)+FLAGVars(10)+FLAGVars(11)
                            do n=1,nC
                   ...
                   Erro_SWOT_Alt_t(i)=Erro_SWOT_Alt(iObs)
                   meanERRO_SWOT_Alt_t(i)=meanERRO_SWOT_Alt(iObs)
                endif
            enddo        
            ...
            ! Correccion matriz de errores
            do iObs=1,nObs_t
                if (Erro_SWOT_Alt_t(iObs)<0.25) then
                    Erro_SWOT_Alt_t(iObs)=0.25
                endif
            enddo
 ...
 !  VERIFICAR ACÁ SI EL ERROR DE SWOT ES EN VALOR ABSOLUTO (quedaria como Alt) O EN PORCENTAJE (quedaria como Q)
		    ...
		    do iEns=1,nEns
			    ...
			    do iObs=1,nObs_t
				    ...
				    wran=gasdev(seed)
				    ! Gera vetor de erros
                    Ekf(iObs,iEns)=wran*ERRO_SWOT_Alt_t(iObs) + meanERRO_SWOT_Alt_t(iObs)
			    enddo
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        ! Gera matriz de covariancia:
            Rkf=0.0
		    do iObs=1,nObs_t
                Rkf(iObs,iObs)=Erro_SWOT_Alt_t(iObs)**2                        ! Altimetria    error en m
            enddo

        endif
    endif
    
	end subroutine EnKF_Measure_SWOT_Alt