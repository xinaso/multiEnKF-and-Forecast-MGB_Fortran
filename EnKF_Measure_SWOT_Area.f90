 !---------------------------------------------------------------------------------
    ...
    ! End of header
    !---------------------------------------------------------------------------------
    subroutine EnKF_Measure_SWOT_Area
	!------------------------------------------------------------------------------
	!-----------------------------------------------------------------------------
    ...
    !----------------------------------------------------------------------
    iStat0=FLAGVars(1)*nC*nU+FLAGVars(2)*nC+FLAGVars(3)*nC+FLAGVars(4)*nC+FLAGVars(5)*nC+FLAGVars(6)*nC+FLAGVars(7)*nC*nU+FLAGVars(8)*nC+FLAGVars(9)*nC  ! Considerando a variavel Area2
    
    if (iT==1) then
        allocate(HAkf(1,1),Ykf(1),Skf(1,1),Dkf(1,1),Ekf(1,1),Erro_SWOT_A_t(1),D2kf(1,1),innov(1),Rkf(1,1),rho(1,1),rhoc(1,1))
    endif
    ...
    if (iT>=iTini_EnKF.and.iT<iTfim_EnKF) then
                ...
                if (allocated(Erro_SWOT_A_t))    deallocate(Erro_SWOT_A_t)
        ...
        nObs_t=0     
        do iObs=1,nObs_SWOT
            if (Obs_SWOT_A(iObs)/=-9999.0) then
        ...
        if (nObs_t<2) then
            allocate(HAkf(1,nEns),Ykf(1),Skf(1,nEns),Dkf(1,nEns),Ekf(1,nEns),Erro_SWOT_A_t(1),D2kf(1,nEns),innov(1),Rkf(1,1),rho(nStatVar,1),rhoc(1,1))   !Julho
        else    
        
            allocate(Ykf(nObs_t),HAkf(nObs_t,nEns),Skf(nObs_t,nEns),Dkf(nObs_t,nEns),Ekf(nObs_t,nEns),Erro_SWOT_A_t(nObs_t),rho(nStatVar,nObs_t),rhoc(nObs_t,nObs_t))
            ...
            do iObs=1,nObs_SWOT
                if (Obs_SWOT_A(iObs)/=-9999.0) then
                    i=i+1
                    Ykf(i)=Obs_SWOT_A(iObs)
                    iC=XminiObs_SWOT_A(iObs)
                    ...
                    Erro_SWOT_A_t(i)=abs(Erro_SWOT_A)
                endif
            enddo        
 ...
 !  VERIFICAR ACÁ SI EL ERROR DE SWOT ES EN VALOR ABSOLUTO (quedaria como Alt) O EN PORCENTAJE (quedaria como Q)
		    ...
		    do iEns=1,nEns
			    ...
			    do iObs=1,nObs_t
				    ...
				    wran=gasdev(seed)
				    ! Gera vetor de erros
				    Ekf(iObs,iEns)=abs(Ykf(iObs))*wran*Erro_SWOT_A_t(iObs)    ! Q error en %
			    enddo
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            Rkf=0.0
		    do iObs=1,nObs_t
                Rkf(iObs,iObs)=(abs(Ykf(iObs))*Erro_SWOT_A_t(iObs))**2        ! Area error en %
            enddo

    endif
    
	end subroutine EnKF_Measure_SWOT_Area