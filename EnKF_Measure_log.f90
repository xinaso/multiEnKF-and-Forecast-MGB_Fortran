 !---------------------------------------------------------------------------------
    ...
    integer:: m,n,ix,jx
    real*8:: gasdev,wran,noise1
    

    !----------------------------------------------------------------------
    iStat0=FLAGVars(1)*nC*nU+FLAGVars(2)*nC+FLAGVars(3)*nC+FLAGVars(4)*nC+FLAGVars(5)*nC+FLAGVars(6)*nC+FLAGVars(7)*nC*nU
        
    if (iT==1) then
        allocate(HAkf(1,1),Ykf(1),Skf(1,1),Dkf(1,1),Ekf(1,1),Erro_Qlog_t(1),D2kf(1,1),innov(1),Rkf(1,1),rho(1,1),rhoc(1,1))
    endif
    ...
    if (iT>=iTini_EnKF.and.iT<iTfim_EnKF) then
                ...
                if (allocated(Erro_Qlog_t))    deallocate(Erro_Qlog_t)
        ...
        if (nObs_t<2) then
            allocate(HAkf(1,nEns),Ykf(1),Skf(1,nEns),Dkf(1,nEns),Ekf(1,nEns),Erro_Qlog_t(1),D2kf(1,nEns),innov(1),Rkf(1,1),rho(nStatVar,1),rhoc(1,1))  !2018loc
        else    
        
            allocate(Ykf(nObs_t),HAkf(nObs_t,nEns),Skf(nObs_t,nEns),Dkf(nObs_t,nEns),Ekf(nObs_t,nEns),Erro_Qlog_t(nObs_t),rho(nStatVar,nObs_t),rhoc(nObs_t,nObs_t))    !2018loc
                ...
                if (Obs_Q(iObs)>=0.0.and.FLAGObs_Q(iObs)==1) then
                    ...
                    IF (Local_INDEX==1) THEN
                        do m=1,FLAGVars(1)*nU+FLAGVars(2)+FLAGVars(3)+FLAGVars(4)+FLAGVars(5)+FLAGVars(6)+FLAGVars(7)*nU+FLAGVars(8)+FLAGVars(9)+FLAGVars(10)+FLAGVars(11)+FLAGVars(12)
                            do n=1,nC
                            rho((m-1)*nC+n,i)=Corr_EnKF(n,iC)    ! Extraccion de matriz rho (p) a partir de Corr_EnKF(yMini,xMini)               !2018loc
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		    ...
		    do iEns=1,nEns
			    ...
			    do iObs=1,nObs_t
			        ...
			        ! Observacoes perturbadas:
			        ! Usando erro lognormal igual à chuva:
			        
				    noise1=(log(Erro_Qlog_t(iObs)**2+1.))**0.5
				    noise1=noise1*wran
                    noise1=exp(noise1)
                    noise1=1.*noise1/(Erro_Qlog_t(iObs)**2+1.)**0.5
                    
                    D2kf(iObs,iEns)=Ykf(iObs)*noise1
                enddo

			    ! Computa erros:
			    Ekf(:,iEns)=D2kf(:,iEns)-Ykf
		    enddo

   		    !*******************************
   		    ! Passa para escala logaritma:
   		    D2kf=log(D2kf)
   		    HAkf=max(HAkf,0.01)
   		    HAkf=log(HAkf)
   		    Ykf=log(Ykf)
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            Rkf=0.0
		    do iObs=1,nObs_t
                Rkf(iObs,iObs)=log(Erro_Qlog_t(iObs)**2+1.)
            enddo

    endif
    
	end subroutine EnKF_Measure_log