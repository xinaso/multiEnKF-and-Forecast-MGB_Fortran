   !---------------------------------------------------------------------------------
    ...
    !  Discussion:
    ! 
    !   This sub-routine is used to perturb the W state variable
    !
    !
    !---------------------------------------------------------------------------------
...
call sample2D(A2,nlon,nlat,nrens,nre,rhVars(1),samp_fix)
...
if (iT==1) ro=0.0
noiseVars(:,iEns,1)=ro*noiseVars(:,iEns,1)+(1.-ro**2)**0.5*noise1

if (iEns==1) write(666166621,*) ro,noiseVars(50,iEns,1),noiseVars(500,iEns,1),noiseVars(676,iEns,1)
noise1=noiseVars(:,iEns,1)



! Aplica equacao 2 de Nijssen e Lettenmaier (2004)
noise1=(log(ErroW**2+1.))**0.5*noise1
noise1=exp(noise1)
noise1=(1.+BIAS)*noise1/(ErroW**2+1.)**0.5

do iU=1,nU
W(:,iU)=W(:,iU)*noise1
enddo

if (iEns==1) write(666166631,*) ro,noise1(50),noise1(500),noise1(676)
