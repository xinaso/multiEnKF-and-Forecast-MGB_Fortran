    ...
    !  Discussion:
    ! 
    !   This sub-routine updates the state variables of the model by using the EnKF equations and localization approach (LEnKF).
    !
    !
    !---------------------------------------------------------------------------------
    ...
    ! End of header
    !---------------------------------------------------------------------------------
    subroutine analysis_new_local(A, R, E, S, D, innov, rho, rhoc, ndim, nrens, nrobs, verbose, truncation,mode,update_randrot)
! Computes the analysed ensemble for A using the EnKF or square root schemes.
   ...
   real*8, intent(in)    :: rho(ndim,nrobs)  ! matrix of correlation coeficients for Local EnKF                            !2018loc
   real*8, intent(in)    :: rhoc(nrobs,nrobs)! matrix of correlation coeficients for Local EnKF compact version            !2018loc
        ...
        !        Evaluate R= S*S` + (nrens-1)*R
         ...
         allocate(C(nrobs,nrobs))                                   !2018loc
          ...
          do j=1,nrobs                                              !2018loc
                do k=1,nrobs
                    C(j,k) = rhoc(j,k)*SSt(j,k) + real(nrens-1)*R(j,k)
                enddo
          enddo
