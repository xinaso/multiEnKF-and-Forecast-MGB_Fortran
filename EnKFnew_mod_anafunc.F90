...
subroutine lowrankE(S,E,nrobs,nrens,nrmin,W,eig,truncation)
   ...
   real*8,    intent(in)  :: S(nrobs,nrens)
   real*8,    intent(in)  :: E(nrobs,nrens)
   real*8,    intent(out) :: W(nrobs,nrmin)
   real*8,    intent(out) :: eig(nrmin)
   real*8,    intent(in)  :: truncation

   real*8 U0(nrobs,nrmin),sig0(nrmin)
   real*8 X0(nrmin,nrens)
   integer i,j

   real*8 U1(nrmin,nrmin),VT1(1,1)
...
! Compute eigenvalue decomposition of R -> Z*eig*Z` 
   integer, intent(in) :: nrobs
   real*8, intent(in)    :: R(nrobs,nrobs)
   real*8, intent(out) :: Z(nrobs,nrobs)
   real*8, intent(out)   :: eig(nrobs)

!#ifdef IBM
!   real*8, allocatable :: ap(:)
!   integer k
!#endif
   real*8  RR(nrobs,nrobs)

   real*8 fwork(8*nrobs)  
...
implicit none
integer, intent(in)    :: nrobs
real*8,    intent(inout) :: eig(nrobs)
real*8,    intent(in)    :: truncation

integer i,nrsigma
real*8 sigsum,sigsum1
logical ex
...
! Generate X2= (I+eig)^{-0.5} * W^T * S
   ...
   real*8, intent(in)    :: W(idim,nrens)
   real*8, intent(in)    :: S(nrobs,nrens)
   real*8, intent(in)    :: eig(idim)
   real*8, intent(out)   :: X2(idim,nrens)
   integer i,j
...
subroutine genX3(nrens,nrobs,nrmin,eig,W,D,X3)
   ...
   integer, intent(in) :: nrobs
   integer, intent(in) :: nrmin
   real*8,    intent(in) :: eig(nrmin)
   real*8,    intent(in) :: W(nrobs,nrmin)
   real*8,    intent(in) :: D(nrobs,nrens)
   !real*8,    intent(out) :: X3(nrobs,nrmin)                       ! 2018
   real*8,    intent(out) :: X3(nrobs,nrens)

   real*8 X1(nrmin,nrobs)
   real*8 X2(nrmin,nrens)
   integer i,j
...
subroutine meanX5(nrens,nrobs,nrmin,S,W,eig,innov,X5)
   ...
   integer, intent(in) :: nrobs
   integer, intent(in) :: nrmin
   real*8, intent(in)    :: W(nrmin,nrmin)
   real*8, intent(in)    :: S(nrobs,nrens)
   real*8, intent(in)    :: eig(nrmin)
   real*8, intent(in)    :: innov(nrobs)
   real*8, intent(out)   :: X5(nrens,nrens)
...
subroutine X5sqrt(X2,nrobs,nrens,nrmin,X5,update_randrot,mode)
   ...
   real*8, intent(in)    :: X2(nrmin,nrens)
   real*8, intent(inout) :: X5(nrens,nrens)
   logical, intent(in) :: update_randrot
   integer, intent(in) :: mode

   real*8 X3(nrens,nrens)
   real*8 X33(nrens,nrens)
   real*8 X4(nrens,nrens)
   real*8 IenN(nrens,nrens)
   real*8, save, allocatable :: ROT(:,:)


   real*8 U(nrmin,1),sig(nrmin),VT(nrens,nrens)
   real*8, allocatable, dimension(:)   :: work,isigma
...
subroutine dumpX3(X3,S,nrobs,nrens)
   ...
   integer, intent(in) :: nrens
   integer, intent(in) :: nrobs
   !real*8,    intent(in) :: X3(nrens,nrens)
   real*8,    intent(in) :: X3(nrobs,nrens)                     ! 2018
   real*8,    intent(in) :: S(nrobs,nrens)
...
subroutine dumpX5(X5,nrens)
   implicit none
   integer, intent(in) :: nrens
   real*8,    intent(in) :: X5(nrens,nrens)
   integer j
   character(len=2) :: tag2
...
subroutine lowrankCinv(S,R,nrobs,nrens,nrmin,W,eig,truncation)
   ...
   real*8 U0(nrobs,nrmin),sig0(nrmin)
   real*8 B(nrmin,nrmin),Z(nrmin,nrmin)
   integer i,j
...
integer, intent(in) :: nrobs
integer, intent(in) :: nrens
real*8, intent(inout) :: B(nrmin,nrmin)
real*8, intent(in)    :: R(nrobs,nrobs)
real*8, intent(in)    :: U0(nrobs,nrmin)
real*8, intent(in)    :: sig0(nrmin)
real*8 X0(nrmin,nrobs)
integer  i,j
...
subroutine svdS(S,nrobs,nrens,nrmin,U0,sig0,truncation)
   ...
   integer, intent(in)  :: nrmin
   real*8,    intent(in)  :: S(nrobs,nrens)
   real*8,    intent(out) :: sig0(nrmin)
   real*8,    intent(in)  :: U0(nrobs,nrmin)
   real*8,    intent(in)  :: truncation

   real*8 S0(nrobs,nrens)
   real*8 VT0(1,1)
   integer ierr
   integer lwork
