    SUBROUTINE FORECAST_flood_READHOT !chili peppers
    ...
    write(*,*)'Reading Water Balance State Variables...'
        ...
        if (IT_current_PQ < 10) then
            ...
            write (hotFilename,hot_format_string)'0000',IT_current_PQ
        elseif (IT_current_PQ < 100) then
            hot_format_string = "(A3,I2)"
            write (hotFilename,hot_format_string)'000',IT_current_PQ
        elseif (IT_current_PQ < 1000) then
            hot_format_string = "(A2,I3)"
            write (hotFilename,hot_format_string)'00',IT_current_PQ
        elseif (IT_current_PQ < 10000) then
            hot_format_string = "(A1,I4)"
            write (hotFilename,hot_format_string)'0',IT_current_PQ
        endif
                
        open(FILHOTSTART_WB,FILE=INPUT_DIRECTORY_HOTSTART // 'StateVars_WB_'// trim(hotFilename) // '.hot',STATUS='old',RECL=nStatVar,FORM='UNFORMATTED',ACCESS='DIRECT')
    ...
    open(FILHOTSTART_Inert,FILE=INPUT_DIRECTORY_HOTSTART // 'StateVars_inertial_'// trim(hotFilename) // '.hot',STATUS='old',RECL=(nStatVar*2),FORM='UNFORMATTED',ACCESS='DIRECT')    
